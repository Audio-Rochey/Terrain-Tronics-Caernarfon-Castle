...
int GS[4]; // Good Steps = for Leifs Game Mode.
...
#define LED_PIN    D3
#define LED_COUNT 52 // remember that there's one on the board. (A WS2812 chip to be exact)
Adafruit_NeoPixel strip(LED_COUNT, LED_PIN, NEO_GRB + NEO_KHZ400);  // - for WS2812
...
int currLED = 0;
int floorArray [LED_COUNT][3]; 
int currentRow = 0; // Leifs Game Current Row
int leifsRed = 166;
int leifsGreen = 147;
int leifsBlue = 28;
...
void loop() {
  ...
  if (irrecv.decode(&results)) {
    ...
    if (results.value == 0xFF42BD) { // Button 7  
      Serial.println(" Button 7 - Next Row! ");
      buttonSEVEN();
    }
    if (results.value == 0xFF4AB5) { // Button 8  
      Serial.println(" Button 8 - Win a skill check. One wrong path chosen! ");
      buttonEIGHT();
    }
    if (results.value == 0xFF52AD) { // Button 9  
      Serial.println(" Button 9 - RESET 5x5 Playfield ");
      buttonNINE();
    }
